...
app.get("/api/admin/users", requireAuth, requirePasswordChangeEnforcement, requireRole(["ADMIN"]), async (req: Request, res: Response): Promise<any> => {
  try {
    ...
    let parsedPage = parseInt(page as string, 10);
    let parsedLimit = parseInt(pageSize as string, 10);
...
// Helper for generating strong passwords
function generateTempPassword(): string {
  ...
  password += num[crypto.randomInt(0, num.length)];
  password += spec[crypto.randomInt(0, spec.length)];
...
app.post("/api/admin/users", requireAuth, requirePasswordChangeEnforcement, requireRole(["ADMIN"]), async (req: Request, res: Response): Promise<any> => {
  try {
    ...
    if (!fullName || !email || !role) {
      return res.status(400).json({ error: { code: "VALIDATION_ERROR", message: "Name, email and role are required." } });
    }
    ...
    if (existing) {
      return res.status(400).json({ error: { code: "VALIDATION_ERROR", message: "Email is already in use." } });
    }
...
// ---------------------------------------------------------------------------
app.patch("/api/admin/users/:id", requireAuth, requirePasswordChangeEnforcement, requireRole(["ADMIN"]), async (req: Request, res: Response): Promise<any> => {
  try {
    ...
    if (targetUserId === currentAdminId) {
      if (role && role !== "ADMIN") {
        return res.status(400).json({ error: { code: "VALIDATION_ERROR", message: "You cannot demote yourself from the ADMIN role." } });
      }
    ...
    if ((role && role !== "ADMIN" && targetUser.role === "ADMIN") || (isActive === false && targetUser.role === "ADMIN" && targetUser.isActive)) {
      const activeAdminsCount = await prisma.user.count({
        where: { role: "ADMIN", isActive: true }
      });
      if (activeAdminsCount <= 1) {
        return res.status(400).json({ error: { code: "VALIDATION_ERROR", message: "Cannot demote or deactivate the last active administrator." } });
      }
    }
...
app.get("/api/tickets/:id/comments", requireAuth, requirePasswordChangeEnforcement, async (req: Request, res: Response): Promise<any> => {
  try {
    ...
    if (req.user!.role === "REQUESTER" && ticket.requesterId !== req.user!.id) {
      return res.status(403).json({ error: { code: "FORBIDDEN_ACCESS", message: "You do not have permission to view this ticket's comments." } });
    }
...
app.post("/api/tickets/:id/comments", requireAuth, requirePasswordChangeEnforcement, async (req: Request, res: Response): Promise<any> => {
  try {
    ...
    if (req.user!.role === "REQUESTER" && ticket.requesterId !== req.user!.id) {
      return res.status(403).json({ error: { code: "FORBIDDEN_ACCESS", message: "You do not have permission to comment on this ticket." } });
    }
...
app.post("/api/tickets/:id/indicate-resolved", requireAuth, requirePasswordChangeEnforcement, async (req: Request, res: Response): Promise<any> => {
  try {
    ...
    if (req.user!.role === "REQUESTER" && ticket.requesterId !== req.user!.id) {
      return res.status(403).json({ error: { code: "FORBIDDEN_ACCESS", message: "You do not have permission to modify this ticket." } });
    }

    const activeStatuses = ["NEW", "OPEN", "IN_PROGRESS", "WAITING_FOR_REQUESTER"];
    if (!activeStatuses.includes(ticket.currentStatus)) {
      return res.status(400).json({ error: { code: "VALIDATION_ERROR", message: "Ticket is not in an active state to indicate resolution." } });
    }
    ...
    const comment = await prisma.publicComment.create({
      data: {
        ticketId,
        authorId: req.user!.id,
        body: "Requester indicated that the problem appears resolved."
      },
      include: { author: { select: { id: true, fullName: true, role: true } } }
    });

    return res.status(201).json({ 
      message: "Indicated resolved successfully.", 
      data: comment 
    });
  } catch (error) {
    console.error(error);
