...
describe("IT Staff Ticket Queue API", () => {
  ...
  it("should sort tickets by itPriority asc (LOW to CRITICAL)", async () => {
    const res = await request(app)
      .get(`/api/staff/tickets?categoryId=${category.id}&sortBy=itPriority&sortOrder=asc`)
      .set("Cookie", staffCookie);
    expect(res.status).toBe(200);
    expect(res.body.data.length).toBe(2);
    // LOW (ticket2) comes before MEDIUM (ticket1)
    expect(res.body.data[0].ticketNumber).toBe(ticket2.ticketNumber);
    expect(res.body.data[1].ticketNumber).toBe(ticket1.ticketNumber);
  });

  it("should sort tickets by itPriority desc (CRITICAL to LOW)", async () => {
    const res = await request(app)
      .get(`/api/staff/tickets?categoryId=${category.id}&sortBy=itPriority&sortOrder=desc`)
      .set("Cookie", staffCookie);
    expect(res.status).toBe(200);
    expect(res.body.data.length).toBe(2);
    // MEDIUM (ticket1) comes before LOW (ticket2)
    expect(res.body.data[0].ticketNumber).toBe(ticket1.ticketNumber);
    expect(res.body.data[1].ticketNumber).toBe(ticket2.ticketNumber);
  });

  it("should sort tickets by status asc (NEW -> IN_PROGRESS)", async () => {
    const res = await request(app)
      .get(`/api/staff/tickets?categoryId=${category.id}&sortBy=status&sortOrder=asc`)
      .set("Cookie", staffCookie);
    expect(res.status).toBe(200);
    expect(res.body.data.length).toBe(2);
    // NEW (ticket1) comes before IN_PROGRESS (ticket2)
    expect(res.body.data[0].ticketNumber).toBe(ticket1.ticketNumber);
    expect(res.body.data[1].ticketNumber).toBe(ticket2.ticketNumber);
  });

  it("should sort tickets by status desc (IN_PROGRESS -> NEW)", async () => {
    const res = await request(app)
      .get(`/api/staff/tickets?categoryId=${category.id}&sortBy=status&sortOrder=desc`)
      .set("Cookie", staffCookie);
    expect(res.status).toBe(200);
    expect(res.body.data.length).toBe(2);
    // IN_PROGRESS (ticket2) comes before NEW (ticket1)
    expect(res.body.data[0].ticketNumber).toBe(ticket2.ticketNumber);
    expect(res.body.data[1].ticketNumber).toBe(ticket1.ticketNumber);
  });
});
