...
| **E2E-02** | E2E | AC-06, AC-11 | Staff ticket lifecycle | Staff finds ticket in queue, updates status | `e2e/lab-03/staff-ticket-flow.spec.ts` | Pass |
| **E2E-03** | E2E | AC-08, AC-10 | Admin user management flow | Admin creates user, attempts self-deactivation | `e2e/lab-03/user-administration.spec.ts` | Pass |
| **MIG-01** | API | Migration | Data Migration & Regression | Verifies Lab 2 tickets/attachments persist and are owned by migrated users | `server/tests/lab-03/migration.test.ts` | Pass |
